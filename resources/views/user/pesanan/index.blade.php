...
@section('judul','tambah pesanan|cafe')
...
<div class="container-fluid">
    <div class="row">
        <div class="col-8">
            <h1 class="mt-3 text-light">Pesanan</h1>
            @if (session('status'))
            <div class="alert alert-success">
                {{session('status')}}
            </div>
            @endif
            <form action="{{route('tpesan')}}" method="post">
                @csrf
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <input type="number"  class="form-control" name="harga" id="harga" placeholder="Masukkan Harga Stuan" required>
                </div>
                <div class="mb-3">
                    <label for="total" class="form-label text-light">Total</label>
                    <input type="number" name="total" class="form-control" id="total"  placeholder="Masukkan Total Yang di pesan" required>
                </div>
                <div class="mb-3">
                    <label for="no_meja" class="form-label text-light">No Meja</label>
                    <input type="number" name="no_meja" class="form-control" id="no_meja"  placeholder="Masukkan Total Yang di pesan" required>
                </div>
                    <input type="hidden" name="nama_pegawai" id="nama_pegawai" value="Arjunanta">
                    <input type="hidden" name="status_meja" id="status_meja" value="1">
                ...
                {{-- <a  href="/pesanan" class="btn btn-lg btn-warning" type="back">Back</a> --}}
            </form>
        </div>
    </div>
</div>  
